...
- **Track proxy** (`/proxy/dash/<vid>/<idx>/<type>/<qn>/<cid>`): `CdnConnection` through WARP; must forward the client `Range` and return `206` + `Content-Range`/`Content-Length`/`Accept-Ranges` (dash.js drives segmentation via sidx itself). Computable lengths are load-bearing for dash.js throughput/ABR/abandon decisions, so `206` responses without a derivable length are skipped as bad mirrors (synthesizing `Content-Length` from `Content-Range` when needed) instead of serving a lengthless stream that ends in "Request timeout: non-computable download size" (dash.js#4716); the player uses a 60s `fragmentRequestTimeout` with the abandon-requests rule explicitly on. Each track is tried on its primary `base_url` first, then its `backup_url` mirrors with an 8s per-attempt header deadline (`DASH_ATTEMPT_TIMEOUT`). Past the handshake, a mid-body watchdog (`DASH_BODY_GRACE` 3s, `DASH_BODY_FLOOR_KBPS` 300KB/s) aborts tarpitting edges and transparently resumes the remaining `Range` on the next mirror (byte-contiguous, invisible to dash.js): some objects answer headers in ms but trickle the body at ~100KB/s while siblings serve in ms, and hanging until dash.js abandons used to surface as bare `500`s with no app log. Exhausted mirrors return logged `502` (handshake phase) or a truncated stream (body phase, the player re-requests); client abandons are logged with elapsed time and re-raised.
- **Frontend:** vendored `static/vjs/dash.min.js` (v5.2.1, via `npm run sync:static` from the `dashjs` package — player code must use the v5 representations API, the v4 `getBitrateInfoListFor`/`setQualityFor` no longer exist); `DashPlayerManager` in `static/themes/modern/js/player.js`; `api_component_player` passes `is_dash`/`dash_url` to the templates.
...
- **Download:** `POST /download` with XHR/fetch creates a background job (`create_download_job` in `dash_proxy.py`, max 3 concurrent, unguessable uuid IDs so many users can download at once) and returns `{"job_id"}`; the floating dialog (`static/themes/modern/js/download.js`) polls `GET /download/status/<job>` (percent, `speed_bps`, phases: queued/resolving/downloading/muxing/ready) and auto-saves `GET /download/file/<job>` when ready. Transient CDN/WARP cuts mid-download are resumed with `Range` requests (5 retries max, few-seconds backoff, surfaced as a status note). Cancel button / dialog close / pagehide (`sendBeacon`) hits `POST /download/cancel/<job>`, which aborts the CDN fetch, kills ffmpeg, and deletes temp files. DASH videos are muxed (best video track ≤ qn 80 = 1080P anonymous cap + best standard audio, `ffmpeg -c copy -movflags +faststart`; requires `ffmpeg` in the app image). durl-only videos download the progressive MP4 directly (already muxed, no ffmpeg step). Plain form posts keep the legacy `302 /proxy/download/<vid>/<idx>/<qual>` streaming path for no-JS clients.

...
- **Track proxy** (`/proxy/dash/<vid>/<idx>/<type>/<qn>/<cid>`): `CdnConnection` through WARP; must forward the client `Range` and return `206` + `Content-Range`/`Content-Length`/`Accept-Ranges` (dash.js drives segmentation via sidx itself). Each track is tried on its primary `base_url` first, then its `backup_url` mirrors with an 8s per-attempt header deadline (`DASH_ATTEMPT_TIMEOUT`): individual objects sometimes stall on one edge for 10-25s+ while siblings serve in ms, and hanging on the primary until dash.js abandons the request used to surface as bare `500`s with no app log. Exhausted mirrors return logged `502`; client abandons are logged with elapsed time and re-raised.

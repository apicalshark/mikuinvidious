...
- **Download:** `POST /download` with XHR/fetch creates a background job (`create_download_job` in `dash_proxy.py`, max 3 concurrent, unguessable uuid IDs so many users can download at once) and returns `{"job_id"}`; the floating dialog (`static/themes/modern/js/download.js`) polls `GET /download/status/<job>` (percent, `speed_bps`, phases: queued/resolving/downloading/muxing/ready) and auto-saves `GET /download/file/<job>` when ready. Cancel button / dialog close / pagehide (`sendBeacon`) hits `POST /download/cancel/<job>`, which aborts the CDN fetch, kills ffmpeg, and deletes temp files. DASH videos are muxed (best video track ≤ qn 80 = 1080P anonymous cap + best standard audio, `ffmpeg -c copy -movflags +faststart`; requires `ffmpeg` in the app image). durl-only videos download the progressive MP4 directly (already muxed, no ffmpeg step). Plain form posts keep the legacy `302 /proxy/download/<vid>/<idx>/<qual>` streaming path for no-JS clients.

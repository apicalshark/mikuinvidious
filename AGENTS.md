...
- **durl-only fallback (verified Sep 11 2026):** `durl` is NOT dead for all uploads — some UGC videos (e.g. `BV1kH35z9EzG`, an 80-min `vupload` MP4) return a `durl`-only playurl with **no `dash` node** on every endpoint/param combo, which left the player stuck on the "正在解析视频地址..." spinner (`is_dash=False` + empty `supported_src`, no manifest request). `video_get_dash_for_qn` now also returns the `durl` node; `has_valid_dash_tracks()` gates the DASH path on a real `SegmentBase.indexRange`; otherwise `fetch_durl_supported_src()` fetches per-quality progressive URLs (non-wbi playurl + PGC retry, parallel, reusing the already-fetched first `durl`) and caches them under `mikuinv_<vid>_<idx>_<qn>` (`+_bak` for backups) so the existing native `/proxy/video/...` path + quality menu work unchanged. **Download:** `/proxy/download/...` falls back for durl-only videos to a `302` redirect at `/proxy/video/<vid>_<idx>_<qn><ext>?dl=1` (best quality ≤ requested; progressive MP4s are already muxed so no ffmpeg step is needed).
